...
// Functions
/* block_stmt_func: LP stmt_func_list (return_stmt |) RP; if_stmt_func: IF LB expr RB (stmt |
 return_stmt) ( ELSE (stmt | return_stmt) | ); stmt_func_list: stmt_func stmt_func_list |;
 stmt_func: init_stmt | asm_stmt | if_stmt_func | for_stmt | while_stmt | dowhile_stmt | break_stmt
 | continue_stmt | call_stmt | block_stmt;
 */
func:
...
expr7: expr8 idx_ops | expr8;
expr8: LB expr_list RB | operands;
...
init_stmt: (variable | expr) SEMICOLON;
asm_stmt: scalar_variable ASM expr SEMICOLON;
if_stmt: IF LB expr RB stmt (ELSE stmt |);
for_stmt:
	FOR LB scalar_variable ASM expr COMMA expr COMMA expr RB stmt;
while_stmt: WHILE LB expr RB stmt;
dowhile_stmt: DO stmt WHILE LB expr RB SEMICOLON;
